var webviewGroup = function(id, options) {
	//options.styles--可以用于设置创建窗体的样式
	//options.nativeStyles--//设置原生窗体过渡的样式
...
proto._initNativeView = function() {
	var styles=mui.extend({
		'top': '84px',//这个需要根据顶部导航及顶部选项卡高度自动调整
	    'bottom':'50px',
		'left': '100%',
		'width': '100%',
		"backgroundColor":"#ffffff"
	},this.options.nativeStyles);	
	this.nativeView = new plus.nativeObj.View('__MUI_TAB_NATIVE_'+this.id, styles);
	try{
		var toLeft= window.screen.width/2 -150/2;
		this.nativeView .draw([
			{tag:'img',id:'img',src:'/resources/images/longlogo.png',
				position:{top:'40%',left:toLeft,width:'150px',height:'50px'}
			},
			/*{tag:'rect',id:'rect',color:'#FF0000',
				position:{top:'0px',left:'0px',width:'100%',height:'1px'}
			},
			{tag:'font',id:'font',text:'原生View控件',textStyles:{size:'18px'},
				position:{top:'0px',left:'0px',width:'100%',height:'1px'}
			}*/
		]);
	}catch(ex){
